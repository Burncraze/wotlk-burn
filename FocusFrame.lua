...
function FocusFrameUpdateBuffAnchor(buffName, index, numFirstRowBuffs, numDebuffs, buffSize, offset, hasTargetofFocus)
	local buff = getglobal(buffName..index);
	
	if ( index == 1 ) then
		if ( UnitIsFriend("player", "focus") ) then
			buff:SetPoint("BOTTOMLEFT", FocusFrame, "TOPLEFT", FocusFrame.buffStartX, FocusFrame.buffStartY);
		else
